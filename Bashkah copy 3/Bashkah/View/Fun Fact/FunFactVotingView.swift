...
struct FunFactVotingView: View {
    ...
    var body: some View {
        ...
        // ✅ FIX: react to phase changes reliably
        .onChange(of: viewModel.gameRoom?.currentPhase) { newPhase in
            ...
            if newPhase == .results {
                ...
                // new voting round/fact
                resetTimer()
            }
        }

        // ✅ FIX: when host reveals answer, stop timer
        .onChange(of: viewModel.showingAnswer) { showing in
            if showing { stopTimer() }
        }

        // ✅ FIX: when fact changes, reset timer & clear local selection UI if needed
        ...
        .onAppear {
            ...
            // Ensure we’re listening for phase changes while inside voting
            // (safe even if already set elsewhere)
            if viewModel.gameRoom?.currentPhase == .voting {
    ...
    // MARK: - Answer Section (FIXED correct check)
    private var answerSection: some View {
        VStack(spacing: 15) {
            if let currentFact = viewModel.currentDisplayFact {
                ...
                // ✅ FIX: compare selectedVote with the real fact owner string id
    ...
    // MARK: - Player Buttons View (FIXED correctness logic)
