...
struct FunFactOpinion: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // ✅ FIX: non-host players must react when host starts writing phase
            // We already have a roomPhaseListener in viewModel.setupWaitingListener,
            // but lobby needs to follow startWriting too.
            startPhaseFollowForLobby()
        }
    }

    // ✅ Minimal “follow phase” without changing UI:
    private func startPhaseFollowForLobby() {
        ...
        viewModel.firebaseManager.db.collection("gameRooms")
            ...
            .addSnapshotListener { snapshot, _ in
                ...
                DispatchQueue.main.async {
                    // When host starts writing phase, everyone should enter writing screen
                    if phase == "writingFacts" {
                        // reset local writing state once when entering
                        viewModel.currentFactIndex = 0
                        viewModel.navigateToWriting = true
                    }
                }
            }
    }
