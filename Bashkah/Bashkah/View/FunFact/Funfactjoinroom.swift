...
struct FunFactJoinRoom: View {
    ...
    private var joinButton: some View {
        Button(action: {
            ...
            ZStack {
                ...
                LinearGradient(
                    colors: [
                        roomNumber.count == 5 ? Color("Orange") : Color("Orange").opacity(0.4),
                        roomNumber.count == 5 ? Color("Orange").opacity(0.8) : Color("Orange").opacity(0.2)
                    ],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
                ...
                .cornerRadius(29)
                .shadow(
                    color: (roomNumber.count == 5 ? Color("Orange") : Color.gray).opacity(0.5),
                    radius: 15,
                    x: 0,
                    y: 8
                )
    ...
    private func joinRoom() {
        guard roomNumber.count == 5, let _ = Int(roomNumber) else {
        ...
        viewModel.joinRoom(roomNumber: roomNumber, playerName: player.name)
