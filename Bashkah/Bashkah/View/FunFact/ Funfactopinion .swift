...
struct FunFactOpinion: View {
    ...
    private var playerCountView: some View {
        HStack(spacing: 10) {
            ...
            Text("\(viewModel.gameRoom?.players.count ?? 0)/8 لاعبين")
    ...
    NavigationStack {
        FunFactOpinion(viewModel: {
            ...
            if var room = vm.gameRoom {
                let player2 = FunFactPlayer(name: "دينا", deviceID: "device2", isReady: true)
                let player3 = FunFactPlayer(name: "نجد", deviceID: "device3", isReady: false)
                let player4 = FunFactPlayer(name: "لينا", deviceID: "device4", isReady: true)
                let player5 = FunFactPlayer(name: "سارة", deviceID: "device5", isReady: true)
                let player6 = FunFactPlayer(name: "مها", deviceID: "device6", isReady: false)
                
                room.players.append(contentsOf: [player2, player3, player4, player5, player6])
                vm.gameRoom = room
            }
